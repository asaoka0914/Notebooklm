...
def build_book_summary_prompt(book_title, author=''):
    ...
    p.append('')
    p.append('請嚴格依照以下【長篇書籍 6 大深度模組】格式輸出：')
    p.append('')
    p.append(f'# 《{book_title}》全書核心精華導讀')
    p.append('')
    p.append('## 📌 一、全書核心 (Executive Summary)')
    p.append('- **一句話主旨**：本書核心欲解決的問題與核心論點。')
    p.append('- **核心主張 (Paradigm Shift)**：舊思維範式 vs. 本書提出的新範式。')
    p.append('- **適用對象與情境**：適合誰閱讀、何種決策場景最具參考價值。')
    p.append('')
    p.append('## 🧠 二、核心心智模型與底層理論 (Core Mental Models & Concepts)')
    p.append('(請提煉 6 ~ 8 個貫穿全書的核心心智模型/重要理論，每個概念請包含機制定義與核心價值。概念連結請一律使用 [[english-slug|中文概念名稱]] 格式，如 [[purpose-teleology|目的論]])')
    p.append('1. **[[concept-slug-a|概念名稱 A]]**：')
    p.append('   - **機制定義**：...')
    p.append('   - **核心價值**：...')
    p.append('2. **[[concept-slug-b|概念名稱 B]]**：')
    p.append('   - **機制定義**：...')
    p.append('   - **核心價值**：...')
    p.append('(以此類推 6~8 項)')
    p.append('')
    p.append('## 🗺️ 三、全書邏輯架構與論證脈絡 (Structural Flow)')
    p.append('- **第一階段：問題診斷**（作者發現的核心盲點與痛點）')
    p.append('- **第二階段：機制解析**（現象背後的運作規律與底層邏輯）')
    ...
    p.append('  2. ...')
    p.append('  3. ...')
    p.append('- **⚠️ 常見誤區 (Don\'ts)**：')
    p.append('  1. ...')
    p.append('  2. ...')
    p.append('  3. ...')
    p.append('- **📋 落地執行清單 (Checklist)**：')
    p.append('  - [ ] ...')
    p.append('  - [ ] ...')
    p.append('')
    p.append('## 💡 五、經典案例、反直覺洞見與金句 (Key Insights & Quotes)')
    p.append('- **🧠 反直覺洞見**：顛覆常識或直覺盲點的深刻觀點。')
    p.append('- **📖 代表性案例 / 數據實驗**：書中支撐核心論點的關鍵案例、歷史典故或數據研究。')
    p.append('- **✍️ 全書精選金句**：')
    p.append('  > 3~5 句極具啟發性、值得反覆咀嚼的原文金句。')
    p.append('')
    p.append('## 🔗 六、Obsidian 概念關聯與延伸閱讀 (Knowledge Graph Connections)')
    p.append('- **🌐 核心概念導引**：[[concept-slug-a|概念1]]、[[concept-slug-b|概念2]]（一律使用 [[concept-slug|中文名稱]] 格式）')
    p.append('- **🗂️ 知識庫關聯主題**：關聯的主題或領域（如 [[adlerian-psychology|阿德勒心理學]]）')
    return '\n'.join(p)
